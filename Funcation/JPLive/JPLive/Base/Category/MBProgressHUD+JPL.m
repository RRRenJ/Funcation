...
@implementation MBProgressHUD (JPL)
/**
 *  显示信息
 *
 *  @param text 信息内容
...
+ (void)show:(NSString *)text icon:(UIImage *)icon view:(UIView *)view{
    ...
    if (!([UIApplication sharedApplication].keyWindow)) {
        hud.offset = CGPointMake(0, -JPL_NAVIGATION_HEIGHT);
    }
    hud.margin = 15;
    if (icon) {
        // 设置图片
...
+ (void)jpl_showMessage:(NSString *)message{
    [self jpl_showMessage:message toView:nil ];
}
...
+ (MBProgressHUD *)jpl_showProgressMessage:(NSString *)message{
    return [self jpl_showProgressMessage:message toView:nil];
}
