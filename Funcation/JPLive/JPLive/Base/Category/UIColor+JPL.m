...
#import "UIColor+JPL.h"

@implementation UIColor (JPL)
...
+ (UIColor *)jpl_colorWithHexString:(NSString *)color{
    return [self jpl_colorWithHexString:color alpha:1.0f];
}
...
+ (CAGradientLayer *)jpl_gradientWith:(CGRect)frame fromeColor:(UIColor *)fromeColor toColor:(UIColor *)toColor fromePoint:(CGPoint)fromePoint toPoint:(CGPoint)toPoint{
    ...
    layer.frame = frame;
    layer.startPoint = fromePoint;
    layer.endPoint = toPoint;
    layer.locations = @[@0,@1];
    layer.colors = @[(__bridge id)fromeColor.CGColor,(__bridge id)toColor.CGColor];
    return layer;
}
